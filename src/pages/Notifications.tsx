    ...
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-6 py-10">
        ...
        <Tabs value={active} onValueChange={(v) => setActive(v as Category)}>
          <TabsList className="w-full md:w-fit gap-2 flex-wrap justify-start h-auto min-h-10">
            <TabsTrigger
              value="all"
              className="rounded-full px-2 sm:px-3 py-1.5 sm:py-2 flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm"
            >
              <Bell className="w-3 h-3 sm:w-4 sm:h-4" /> All
            </TabsTrigger>
            <TabsTrigger
              value="transactions"
              className="rounded-full px-2 sm:px-3 py-1.5 sm:py-2 flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm"
            >
              <ArrowDownLeft className="w-3 h-3 sm:w-4 sm:h-4" /> Transactions
            </TabsTrigger>
            <TabsTrigger
              value="kyc"
              className="rounded-full px-2 sm:px-3 py-1.5 sm:py-2 flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm"
            >
              <ShieldCheck className="w-3 h-3 sm:w-4 sm:h-4" /> KYC Updates
            </TabsTrigger>
            <TabsTrigger
              value="announcements"
              className="rounded-full px-2 sm:px-3 py-1.5 sm:py-2 flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm"
            >
              <Megaphone className="w-3 h-3 sm:w-4 sm:h-4" /> Announcements
            </TabsTrigger>
          </TabsList>
        </Tabs>
